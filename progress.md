...
## 会话：2026-08-17（同名导入自动改名 + 开机自动启动）

- 同名资源可管理（决策：Windows 风格数字后缀、绝不覆盖）：`ManagedLibrary.importResource` 目标目录同名时自动改名为首个空闲的 `名称 (2).ext`（保留扩展名；文件夹/无扩展名整体追加 ` (2)`），复制/移动/命名模板（targetName）路径一致，最终名同样校验无分隔符；退出管理（恢复原路径/移动到指定位置/回收站）、撤销、整理与空间包导入保持不覆盖拒绝。新增库助手 `expectedImportName`（isFolder + extraOccupiedNames 批次占用），导入对话框与真实导入共用同一解析逻辑。
- 导入对话框来源列表在将被改名的来源旁标注 `→ 名称 (2).ext`（来源/标签/模板开关/目标目录变化时异步刷新，批次内先解析名称计入占用，预览与顺序导入一致），改名不再静默。
- 设置 → Windows 集成新增「开机自动启动」行（PillSwitch）：`UserPreferences.autoStartEnabled`（默认 false，fromJson 宽容），updatePreferences 记录「开机自动启动 开启/关闭」；原生 `tagtag/windows_close_behavior` 通道新增 setAutoStart/isAutoStart，写/删 HKCU `...\CurrentVersion\Run` 的 `TAGTAG` 值（GetModuleFileNameW 加引号路径）；保存设置后应用，启动时按持久化偏好自愈重写（应对便携包移动），开启失败弹告警；安装器加 `uninsdeletevalue` 卸载清理 Run 值。
- 测试：库层新增 4 个（复制三连改名、移动改名删源、文件夹整体改名、expectedImportName 解析含缺口跳过/批次占用/名称校验），原「重命名导入不覆盖」测试改写为断言自动改名；新增 test/windows_auto_start_test.dart 5 个（通道转发、偏好往返+legacy 宽容、设置日志、原生/安装器源码扫描、设置行开关保存 widget）；导入标注 widget 测试 1 个。
- 最终门禁：analyze 0 问题、全量测试 158/158（基线 148 + 净增 10）、Windows Release 构建成功（首次因 sqlite3.dll 被测试进程瞬时占用失败，重试通过）；same-name-and-autostart 四个工单与 map 均已 resolved。未做 git 提交。

## 会话：2026-08-18（右键菜单 / 同名策略定稿 / 开机启动）

- 资源右键菜单：全部资源、最近、搜索、待整理、标签层级结果行统一支持右键，先选中再弹出；条目为打开/定位/添加新标签/修改标签/重命名/三项退出管理，复用行内动作回调。
- 同名文件管理按用户定稿实现：导入重名时优先用直接标签名做后缀（name-品牌.txt），其次 MM-DD 日期后缀，序号 (2) 为最后保底；导入后弹出可渐隐 SnackBar 告知并提供「重命名」入口。
- 重命名对话框：文本框预填当前名，时间/TAG/序号三个后缀筹码只改文本不保存，保存时校验（非空、非法字符、绝不覆盖），ManagedLibrary 新增受管 rename 操作（schema v8、事务、嵌套路径更新、领域层撤销）。
- 开机自动启动：HKCU Run 键注册/删除（当前用户免管理员），autoStartEnabled 偏好 + Windows 集成设置行 + 启动时同步。
- 过程中修正上一代理被拒的纯序号方案；最终门禁：分析 0 问题、测试 161/161、Release 构建成功。

## 会话：2026-08-18（六项交互修正）

- 重命名后缀筹码改为基于原始名称的状态机：每个筹码从原名生成对应后缀（不再叠加到文本框内容），序号筹码连击递增。
- Toast 文案统一去掉结尾句号。
- 弹出菜单动画经 popUpAnimationStyle 提速（300ms → 120ms/90ms），接入右键菜单、标签操作、空间切换、导入、更多操作、日志筛选等全部菜单。
- 同名标签徽标由“同名”改为 #身份后四位（如 #93b7），真正可区分同名实体。
- 空间色块按空间 ID 哈希稳定取色，切换空间时切换器与菜单颜色随空间变化。
- 标签层级页新增右键菜单：节点（新建子标签/重命名标签/更改上级标签…/删除标签，先选中）与空白区域（新建根标签）。
- 全量测试 162/162，分析 0 问题，Windows Release 构建成功。

## 会话：2026-08-18（顶部紧凑 Toast 与撤销动作）

- Toast 从底部 SnackBar 改为页面顶部居中的紧凑胶囊（AppToast，maxWidth 460，1.8s 渐隐），支持动作按钮。
