...
- 2026-08-11：用户确认发布版本遵循 `MAJOR.MINOR.PATCH`。项目仍处于 `0.y.z` 开发阶段；`1.0.0` 仅在明确稳定发布时使用。
- 2026-08-11：最新成功发布为 `v0.6.1`，而 `pubspec.yaml` 仍声明 `1.0.0+1`，两者冲突；应先校正为 `0.6.1+1`。历史 `v0.2.0` 至 `v0.6.0` 虽曾将修复按 MINOR 发布，但已公开的标签和 Release 不重写。
- 2026-08-11：版本规则属于发布工程策略，不是 TAGTAG 领域语言，因此不写入 `CONTEXT.md`；应写入本地发布规格、README 和自动化校验。
- 2026-08-11：受管存储工单 01 和 03 的验收内容已经由 schema v6、事务迁移、显式一致性告警、冲突安全撤销、在线 SQLite 快照、SHA-256 校验和空目标恢复覆盖；它们是状态滞留，不是缺失功能。
- 2026-08-11：需求审计文档第 7 节仍描述原型期风险，已与当前实现明显脱节；下一功能不能依据该节的“未实现”字样直接选择，必须从当前公开接口和测试核对。
